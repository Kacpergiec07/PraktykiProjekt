  ...
  <div class="from-blue-50 to-indigo-100 p-6 rounded-x1 mb-20">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <drug-item
        ...
        @add-to-cart="openCartModal(drug)"
      />
    </div>
...
export default {
  ...
  methods: {
    ...
    submitCart() {
      const maxAvailable = this.selectedDrug.amount;
      const amountToAdd = parseInt(this.cartAmount);

      if (amountToAdd > 0 && amountToAdd <= maxAvailable) {
