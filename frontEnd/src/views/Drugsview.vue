<template>
  <div>
    ...
    <div class="relative z-10">
      ...
      <div v-if="isSearchActive" class="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded">
      <div class="flex justify-between items-center">
        <div>
          <p class="font-medium">Wyniki wyszukiwania dla: "{{ searchQuery }}"</p>
        </div>
        <button
          @click="clearSearch"
    ...
    >
      <div class="bg-white rounded-lg max-w-lg w-full shadow-xl transform transition-all duration-300">
        <!-- Modal Header -->
        ...
        <div class="p-6">
          <form @submit.prevent="performSearch">
            <div class="mb-4">
              <label class="block text-sm font-medium text-gray-700 mb-2">Wyszukaj według</label>
              <div class="flex space-x-2 mb-4">
                <label class="inline-flex items-center">
                  <input type="radio" v-model="searchType" value="name" class="form-radio text-blue-600">
                  <span class="ml-2">Nazwa leku</span>
                </label>
                <label class="inline-flex items-center">
                  <input type="radio" v-model="searchType" value="companyName" class="form-radio text-blue-600">
                  <span class="ml-2">Producent</span>
                </label>
                <label class="inline-flex items-center">
                  <input type="radio" v-model="searchType" value="type" class="form-radio text-blue-600">
              ...
              <div class="mb-4">
                <label class="flex items-center">
                  <input type="checkbox" v-model="exactMatch" class="form-checkbox text-blue-600">
                  <span class="ml-2">Dokładne dopasowanie</span>
                </label>
                <p class="text-xs text-gray-500 mt-1">Zaznacz, aby szukać tylko dokładnych wartości</p>
              </div>
  ...
  methods: {
    ...
    async performSearch() {
      ...
      try {
        ...
        console.log(`Searching for ${this.searchType}: "${query}" (Exact match: ${exactMatch})`);
        ...
        if (!this.drugs || this.drugs.length === 0) {
          ...
          let errorMessage = "";
          const matchType = exactMatch ? "dokładnie pasującej do" : "zawierającej";
        ...
        this.error = "Błąd podczas wyszukiwania: " + (error.message || "Nieznany błąd");
      ...
      setTimeout(() => {
        ...
        if (drugItems && drugItems.length > 0) {
            ...
            if (this.searchType === 'name') {
              ...
              if (nameElement) {
                ...
                if (this.exactMatch) {
                  ...
                  matchFound = name.toLowerCase().includes(searchQuery.toLowerCase());
                }
              }
            } else if (this.searchType === 'companyName') {
              // Szukaj w producencie (paragraf zawierający "Producent:")
              const companyElements = Array.from(item.querySelectorAll('p'));
              const companyElement = companyElements.find(p => p.textContent.includes('Producent:'));
              ...
              if (companyElement) {
                ...
                if (this.exactMatch) {
                  // Dokładne dopasowanie (case-insensitive)
                  matchFound = company.toLowerCase() === searchQuery.toLowerCase();
                } else {
                  // Częściowe dopasowanie
                  matchFound = company.toLowerCase().includes(searchQuery.toLowerCase());
                }
              }
            } else if (this.searchType === 'type') {
              // Szukaj w typie leku (paragraf zawierający "Typ:")
              const typeElements = Array.from(item.querySelectorAll('p'));
              const typeElement = typeElements.find(p => p.textContent.includes('Typ:'));
              ...
              if (typeElement) {
                ...
                if (this.exactMatch) {
                  ...
                  // Częściowe dopasowanie
                  matchFound = type.toLowerCase().includes(searchQuery.toLowerCase());
                }
              }
            }
          ...
          if (matchingItems.length > 0) {
            ...
            console.log(`Znaleziono i podświetlono ${matchingItems.length} pasujących elementów`);
          } else {
            console.log("Nie znaleziono elementów pasujących do wyszukiwania w DOM");
          }
        }
